...
async def unmute_cmd(message: types.Message):
   try:
      ...
      if admin.status in {'administrator', 'creator'}:
         ...
         return
      await bot.restrict_chat_member(message.chat.id, message.reply_to_message.from_user.id, can_send_messages=True, can_send_other_messages=True, can_add_web_page_previews=True)
