...
async def promote_cmd(message: types.Message, command: Command):
   ...
   if member.status not in {'administrator', 'creator'}:
      ...
      await bot.promote_chat_member(message.chat.id, message.reply_to_message.from_user.id, can_manage_chat=True, can_delete_messages=True, can_restrict_members=True, can_invite_users=True, can_pin_messages=True, can_manage_video_chats=True, can_promote_members=True)
